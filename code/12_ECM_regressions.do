...
using "$outpath/sd_data.csv", novarnames replace
...
collect export "$outpath/coint1.tex", name(pedroni_table) tableonly replace
...
foreach y of varlist lncapital lntotcur {
	...
	*ECM Capital:
	...
	estimates store e`y'
	*outreg2 using "$outpath/reg1.tex",  dec(3) replace ctitle(ln_capital) 
	...
	*outreg2 using "$outpath/reg1.tex",  dec(3) append ctitle(ln_capital) 
	
	if "`y'"=="lncapital" {
		estout e`y' ed`y' using "$outpath/reg_main_`y'.tex", replace drop(_cons) style(tex) ///
...
*****************************************************************************
	...
	*outreg2 using "$outpath/reg2.tex",  dec(3) replace ctitle(ln_totcur) 
	...
	*outreg2 using "$outpath/reg2.tex",  dec(3) append ctitle(ln_totcur) 
	*/
...
foreach y of varlist lncapital lntotcur {
	forvalues i=0/1 {
		...
		*outreg2 using "$outpath/reg3.tex",  dec(3) replace ctitle(ln_capital) 
		...
		*outreg2 using "$outpath/reg3.tex",  dec(3) append ctitle(ln_capital)
		
		drop resid
			
		if "`y'"=="lncapital" {
			estout e`y'`i' ed`y'`i' using "$outpath/reg_`y'_by_debt_frequency_`i'.tex", replace drop(_cons) style(tex) ///
			cells(b(star fmt(3)) se(par fmt(3))) mlabels(none) collabels(none) ///
			label starlevels(* .1 ** .05 *** .01) prehead(`"& Log of Capital & $\Delta$ Log of Capital \\ \hline "') ///
				stats(N_g N , fmt(%9.0g %9.0g) labels("School Districts" "Observations")) 
		}
...
forvalues i=0/1 {
	...
	*ECM Current Exp:
	xtreg lntotcur lnincomecnty lnenrollment lnpop if belowp50==`i', fe 
		*outreg2 using "$outpath/reg4.tex",  dec(3) replace ctitle(ln_totcur) 

	predict resid2, residuals

	reg d.lntotcur l.resid2 d.l(0/4).lnincomecnty d.l(1/4).lntotcur d.l(0/4).lnenrollment d.l(0/4).ln_pop if belowp50==`i' // ECM-2
	*outreg2 using "$outpath/reg4.tex",  dec(3) append ctitle(ln_totcur) 
...
outreg2 using "$outpath/reg5.tex",  dec(3) replace ctitle(ln_capital) 
...
outreg2 using "$outpath/reg5.tex",  dec(3) append ctitle(ln_capital) 


*ECM Current Exp:
xtreg ln_totcur ln_income_cnty ln_enrollment ln_pop, fe 
outreg2 using "$outpath/reg6.tex",  dec(3) replace ctitle(ln_totcur) 
...
outreg2 using "$outpath/reg6.tex",  dec(3) append ctitle(ln_totcur) 
